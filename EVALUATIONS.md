...
Dropped as a meaningful methodological axis (was: separate baselines per batch depending on how curated it seemed). Every image that reaches this repo has already survived the artist's own keep/delete decisions out of a source collection in the thousands — that survivorship bias is constant and total regardless of which batch a piece lands in, so treating "Batch 1 vs. later batches" as different baselines was overthinking a distinction the artist doesn't actually draw. Score every image on the rubric's own terms, no batch-position adjustment. If a specific batch has notable framing worth knowing (a themed pull, one generation session, etc.), that's just useful context for the verdict prose, not a baseline shift.
